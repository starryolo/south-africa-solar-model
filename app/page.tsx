...
export default function HomePage() {
  ...
  // 计算光伏板面积
  const panelArea = GCL_645W_SPECS.area * config.pvPanelCount
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-50 w-full border-b-2 border-secondary bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-foreground">商业光储充投资模型</h1>
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8 rounded-lg border-2 border-primary bg-primary/5 p-6">
          ...
          <div className="grid gap-4 md:grid-cols-4">
            <div>
              ...
              <span className="font-medium">{GCL_645W_SPECS.power}W</span>
            </div>
            <div>
              <span className="text-muted-foreground">转换效率：</span>
              <span className="font-medium">{(GCL_645W_SPECS.efficiency * 100).toFixed(1)}%</span>
            </div>
            <div>
              <span className="text-muted-foreground">单块面积：</span>
              <span className="font-medium">{formatNumber(GCL_645W_SPECS.area, 2)} m²</span>
            </div>
            <div>
        ...
        {/* 储能产品规格说明 */}
        <div className="mb-8 rounded-lg border-2 border-secondary bg-secondary/5 p-6">
          <h2 className="text-lg font-bold text-foreground mb-4">阳光电源储能电池参数</h2>
          <div className="grid gap-4 md:grid-cols-4">
            <div>
              <span className="text-muted-foreground">单块容量：</span>
              <span className="font-medium">{SUNGROW_BATTERY_SPECS.capacity} kWh</span>
            </div>
            <div>
              <span className="text-muted-foreground">标称电压：</span>
              <span className="font-medium">{SUNGROW_BATTERY_SPECS.voltage} V</span>
            </div>
            <div>
              <span className="text-muted-foreground">放电深度：</span>
              <span className="font-medium">{(SUNGROW_BATTERY_SPECS.depthOfDischarge * 100).toFixed(0)}%</span>
            </div>
            <div>
              <span className="text-muted-foreground">循环寿命：</span>
              <span className="font-medium">{SUNGROW_BATTERY_SPECS.cycleLife} 次</span>
            </div>
          </div>
        </div>
        ...
        <Tabs defaultValue="system" className="space-y-8">
          ...
          <TabsContent value="system" className="space-y-6">
            <div className="mb-4">
              ...
              <p className="text-muted-foreground">配置光伏、储能、充电桩和逆变器的规格与数量</p>
            </div>
          ...
          <TabsContent value="financial" className="space-y-6">
            <div className="mb-4">
              ...
              <p className="text-muted-foreground">设置电价、税务和投资相关参数</p>
            </div>
        ...
        <div className="mt-12 rounded-lg border border-border bg-muted/30 p-6 text-sm text-muted-foreground">
          ...
          <ul className="list-disc list-inside space-y-1">
            <li>本模型基于商业用电环境和税收政策设计</li>
            <li>企业所得税率为 25%，增值税 (VAT) 为 13%</li>
            <li>可再生能源投资可享受加速折旧优惠政策</li>
            <li>储能系统采用阳光电源产品，可提供备用电源价值</li>
      ...
      <footer className="border-t border-border bg-muted/30 py-6 mt-12">
        <div className="container mx-auto px-4 text-center text-sm text-muted-foreground">
          <p>商业光储充投资经济模型计算器</p>
          <p className="mt-1">Commercial PV + Storage + EV Charging Investment Model</p>
        </div>
      </footer>
    </div>
  )
